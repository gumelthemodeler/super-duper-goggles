...
local function SwitchToPvPMenu(parentFrame)
	...
	for _, desc in ipairs(playerGui:GetDescendants()) do
		if desc.Name == "PvPLobby" and desc:IsA("Frame") then
			pvpLobby = desc
			break
		end
	end
	...
	if _G.AOT_SwitchTab then 
		_G.AOT_SwitchTab("PvP") 
		_G.AOT_SwitchTab("PVP")
	end
end

function BattleTab.Init(parentFrame)
	-- [[ DECAL CONFIGURATION TABLE ]]
	local DECALS = {
		Modes = {
			Nightmare = "rbxassetid://0",
			Expeditions = "rbxassetid://0",
			Paths = "rbxassetid://0",
			PvP = "rbxassetid://0",
			Raid = "rbxassetid://0",
			WorldBoss = "rbxassetid://0"
		},
		Bosses = {
			-- Nightmare Hunts
			["Frenzied Beast"] = "rbxassetid://0",
			["Abyssal Armored"] = "rbxassetid://0",
			["Doomsday Apparition"] = "rbxassetid://0",

			-- Raids
			["Raid_Part1"] = "rbxassetid://0",
			["Raid_Part2"] = "rbxassetid://0",
			["Raid_Part3"] = "rbxassetid://0",
			["Raid_Part4"] = "rbxassetid://0",
			["Raid_Part5"] = "rbxassetid://0",
			["Raid_Part8"] = "rbxassetid://0",

			-- World Bosses
			["Rod Reiss Titan"] = "rbxassetid://0",
			["Lara Tybur"] = "rbxassetid://0",
			["Doomsday Titan"] = "rbxassetid://0",
			["Ymir Fritz"] = "rbxassetid://0"
		}
	}
	...
	TopNav.Size = UDim2.new(1, 0, 0, 60)
	...
	local function CreateSubNavBtn(name, text)
		...
		btn.MouseButton1Click:Connect(function()
			for k, v in pairs(SubBtns) do 
				TweenService:Create(v, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(150, 150, 150), BackgroundColor3 = Color3.fromRGB(30, 30, 35)}):Play() 
			end
			...
			if name == "Modes" and SelectionContainer.Visible then
				...
				ModesContainer.GroupTransparency = 0
	...
	CreateSubNavBtn("Modes", "GAME MODES")

	-- ==========================================
	-- [[ 2. CAMPAIGN TAB (Linear Story) ]]
	-- ==========================================
	SubTabs["Campaign"] = Instance.new("Frame", ContentArea)
	...
	CampContainer.Size = UDim2.new(0.8, 0, 0.8, 0)
	CampContainer.Position = UDim2.new(0.5, 0, 0.5, 0)
	CampContainer.AnchorPoint = Vector2.new(0.5, 0.5)
	...
	CampImage.Size = UDim2.new(1, 0, 0.65, 0)
	...
	CampTitle.Size = UDim2.new(0.7, 0, 0, 40)
	CampTitle.Position = UDim2.new(0, 30, 0.65, 20)
	CampTitle.BackgroundTransparency = 1
	CampTitle.Font = Enum.Font.GothamBlack
	CampTitle.TextSize = 32
	...
	CampDesc.Size = UDim2.new(0.65, 0, 0, 60)
	CampDesc.Position = UDim2.new(0, 30, 0.65, 65)
	CampDesc.BackgroundTransparency = 1
	CampDesc.Font = Enum.Font.GothamMedium
	CampDesc.TextSize = 16
	...
	PlayCampBtn.Size = UDim2.new(0.25, 0, 0, 60)
	PlayCampBtn.Position = UDim2.new(1, -30, 1, -30)
	PlayCampBtn.AnchorPoint = Vector2.new(1, 1)
	PlayCampBtn.Font = Enum.Font.GothamBlack
	PlayCampBtn.TextSize = 24
	...
	-- ==========================================
	-- [[ 3. GAME MODES TAB & SELECTION MENU ]]
	-- ==========================================
	...
	ModesContainer.Size = UDim2.new(1, 0, 1, 0)
	...
	-- [[ SELECTION CAROUSEL SETUP (PC FIXED SIZE) ]]
	BackBtn = Instance.new("TextButton", SelectionContainer)
	BackBtn.Size = UDim2.new(0, 200, 0, 40)
	BackBtn.Position = UDim2.new(0, 30, 0, 10)
	BackBtn.Font = Enum.Font.GothamBold
	BackBtn.TextSize = 14
	BackBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	BackBtn.Text = "<- BACK TO MODES"
	ApplyButtonGradient(BackBtn, Color3.fromRGB(60, 60, 70), Color3.fromRGB(30, 30, 40), Color3.fromRGB(80, 80, 90))

	BackBtn.MouseButton1Click:Connect(function()
		FadeSwitch(SelectionContainer, ModesContainer)
	end)

	SelectionTitle = Instance.new("TextLabel", SelectionContainer)
	SelectionTitle.Size = UDim2.new(1, -250, 0, 40)
	SelectionTitle.Position = UDim2.new(0, 250, 0, 10)
	SelectionTitle.BackgroundTransparency = 1
	SelectionTitle.Font = Enum.Font.GothamBlack
	SelectionTitle.TextSize = 28
	SelectionTitle.TextColor3 = Color3.fromRGB(255, 215, 100)
	SelectionTitle.TextXAlignment = Enum.TextXAlignment.Left

	SelectionCarousel = Instance.new("ScrollingFrame", SelectionContainer)
	SelectionCarousel.Size = UDim2.new(1, 0, 1, -60)
	SelectionCarousel.Position = UDim2.new(0, 0, 0, 60)
	SelectionCarousel.BackgroundTransparency = 1
	SelectionCarousel.ScrollingDirection = Enum.ScrollingDirection.X
	SelectionCarousel.ScrollBarThickness = 6

	local sListLayout = Instance.new("UIListLayout", SelectionCarousel)
	sListLayout.FillDirection = Enum.FillDirection.Horizontal
	sListLayout.SortOrder = Enum.SortOrder.LayoutOrder
	sListLayout.Padding = UDim.new(0, 35)
	sListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	sListLayout.VerticalAlignment = Enum.VerticalAlignment.Center

	sListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		SelectionCarousel.CanvasSize = UDim2.new(0, sListLayout.AbsoluteContentSize.X + 80, 0, 0)
	end)

	local function OpenSelectionMenu(titleText, dataTable, onPlayFunc)
		SelectionTitle.Text = titleText

		for _, child in ipairs(SelectionCarousel:GetChildren()) do
			if child:IsA("GuiObject") then child:Destroy() end
		end

		local sortedKeys = {}
		for k in pairs(dataTable) do table.insert(sortedKeys, k) end
		...
		for _, key in ipairs(sortedKeys) do
			...
			card.Size = UDim2.new(0, 300, 0, 450) -- PC Exact Size
			...
			bgImg.Image = DECALS.Bosses[key] or "rbxassetid://0" 
			...
			nLbl.Size = UDim2.new(1, -40, 0, 35)
			nLbl.Position = UDim2.new(0, 20, 1, -165)
			...
			nLbl.Text = data.Name or key

			local rLbl = Instance.new("TextLabel", card)
			rLbl.Size = UDim2.new(1, -40, 0, 20)
			rLbl.Position = UDim2.new(0, 20, 1, -125)
			rLbl.BackgroundTransparency = 1
			rLbl.Font = Enum.Font.GothamBold
			rLbl.TextColor3 = Color3.fromRGB(200, 200, 200)
			rLbl.TextSize = 14
			rLbl.TextXAlignment = Enum.TextXAlignment.Left
			rLbl.Text = "Req: Prestige " .. (data.Req or 0)

			local dLbl = Instance.new("TextLabel", card)
			dLbl.Size = UDim2.new(1, -40, 0, 45)
			dLbl.Position = UDim2.new(0, 20, 1, -100)
			...
			local descText = data.Desc or ("HP: " .. (data.Health or "Unknown"))
			if data.Drops then
				descText = descText .. " | Rewards: +" .. (data.Drops.XP or 0) .. " XP, +" .. (data.Drops.Dews or 0) .. " Dews"
			end
			dLbl.Text = descText

			local pBtn = Instance.new("TextButton", card)
			pBtn.Size = UDim2.new(1, -40, 0, 40)
			pBtn.Position = UDim2.new(0, 20, 1, -50)
			pBtn.Font = Enum.Font.GothamBlack
			pBtn.TextSize = 16
			pBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
			pBtn.Text = "DEPLOY"
			ApplyButtonGradient(pBtn, Color3.fromRGB(200, 60, 60), Color3.fromRGB(100, 30, 30), Color3.fromRGB(80, 20, 20))
			pBtn.ZIndex = 5

			card.MouseEnter:Connect(function()
				TweenService:Create(card, TweenInfo.new(0.2), {Size = UDim2.new(0, 315, 0, 475)}):Play()
				TweenService:Create(stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(255, 100, 100)}):Play()
			end)
			card.MouseLeave:Connect(function()
				TweenService:Create(card, TweenInfo.new(0.2), {Size = UDim2.new(0, 300, 0, 450)}):Play()
				TweenService:Create(stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(60, 60, 70)}):Play()
			end)

			pBtn.MouseButton1Click:Connect(function()
				PlayTransition("DEPLOYING...", function()
					onPlayFunc(key)
				end)
			end)
	...
	carouselLayout.Padding = UDim.new(0, 35)
	...
	local function CreateModeCard(name, desc, imgId, layoutOrder, onClickFunc)
		...
		title.Position = UDim2.new(0, 20, 1, -95)
		...
		descLbl.Position = UDim2.new(0, 20, 1, -55)
		...
		card.MouseEnter:Connect(function()
			TweenService:Create(card, TweenInfo.new(0.2), {Size = UDim2.new(0, 315, 0, 475)}):Play()
			TweenService:Create(stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(255, 215, 100)}):Play()
		end)
		card.MouseLeave:Connect(function()
			TweenService:Create(card, TweenInfo.new(0.2), {Size = UDim2.new(0, 300, 0, 450)}):Play()
			TweenService:Create(stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(60, 60, 70)}):Play()
	...
	CreateModeCard("NIGHTMARE HUNTS", "Face corrupted Titans to obtain legendary Cursed Weapons.", DECALS.Modes.Nightmare, 1, function()
		OpenSelectionMenu("SELECT NIGHTMARE HUNT", EnemyData.NightmareHunts, function(bossId)
			Network:WaitForChild("CombatAction"):FireServer("EngageNightmare", { BossId = bossId })
		end)
	end)

	CreateModeCard("EXPEDITIONS", "Explore beyond the walls to scavenge vital resources.", DECALS.Modes.Expeditions, 2, function()
		PlayTransition("DEPARTING ON EXPEDITION...", function()
			Network:WaitForChild("CombatAction"):FireServer("EngageEndless")
		end)
	end)

	CreateModeCard("THE PATHS", "Traverse the endless desert of Ymir to unlock stat points.", DECALS.Modes.Paths, 3, function()
		PlayTransition("CONNECTING TO THE COORDINATE...", function()
			Network:WaitForChild("CombatAction"):FireServer("EngagePaths")
		end)
	end)

	CreateModeCard("PVP ARENA", "Test your ODM combat skills against other players.", DECALS.Modes.PvP, 4, function()
		SwitchToPvPMenu(parentFrame)
	end)

	CreateModeCard("MULTIPLAYER RAID", "Deploy your party to take down Colossal threats.", DECALS.Modes.Raid, 5, function()
		OpenSelectionMenu("SELECT RAID BOSS", EnemyData.RaidBosses, function(raidId)
			Network:WaitForChild("RaidAction"):FireServer("DeployParty", { RaidId = raidId })
		end)
	end)

	CreateModeCard("WORLD BOSS", "A catastrophic threat has appeared. Intercept immediately.", DECALS.Modes.WorldBoss, 6, function()
		OpenSelectionMenu("SELECT WORLD BOSS", EnemyData.WorldBosses, function(bossId)
			Network:WaitForChild("CombatAction"):FireServer("EngageWorldBoss", { BossId = bossId })
		end)
	end)

	carouselLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		ModesCarousel.CanvasSize = UDim2.new(0, carouselLayout.AbsoluteContentSize.X + 80, 0, 0)
	end)

	-- ==========================================
	-- [[ 4. PARTY MENU UI (PC Centered Modal) ]]
	-- ==========================================
	PartyToggleBtn = Instance.new("TextButton", ModesContainer)
	PartyToggleBtn.Size = UDim2.new(0, 250, 0, 45)
	PartyToggleBtn.Position = UDim2.new(0.5, 0, 1, -25)
	PartyToggleBtn.AnchorPoint = Vector2.new(0.5, 1)
	PartyToggleBtn.Font = Enum.Font.GothamBlack
	PartyToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	PartyToggleBtn.TextSize = 16
	PartyToggleBtn.Text = "PARTY / CO-OP"
	...
	PartyContainer.Size = UDim2.new(0, 320, 0, 420) -- PC Fixed Size
	PartyContainer.Position = UDim2.new(0.5, 0, 0.5, 0)
	PartyContainer.AnchorPoint = Vector2.new(0.5, 0.5)
	PartyContainer.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
	PartyContainer.Visible = false
	PartyContainer.ZIndex = 500
	Instance.new("UICorner", PartyContainer).CornerRadius = UDim.new(0, 12)
	Instance.new("UIStroke", PartyContainer).Color = Color3.fromRGB(80, 120, 200)
	Instance.new("UIStroke", PartyContainer).Thickness = 2

	local pHeader = Instance.new("TextLabel", PartyContainer)
	pHeader.Size = UDim2.new(1, 0, 0, 45)
	pHeader.BackgroundTransparency = 1
	pHeader.Font = Enum.Font.GothamBlack
	pHeader.TextColor3 = Color3.fromRGB(255, 215, 100)
	pHeader.TextSize = 18
	pHeader.Text = "PARTY MANAGEMENT"

	local closePartyBtn = Instance.new("TextButton", PartyContainer)
	closePartyBtn.Size = UDim2.new(0, 30, 0, 30)
	closePartyBtn.Position = UDim2.new(1, -10, 0, 10)
	closePartyBtn.AnchorPoint = Vector2.new(1, 0)
	closePartyBtn.BackgroundTransparency = 1
	closePartyBtn.Font = Enum.Font.GothamBlack
	closePartyBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
	closePartyBtn.TextSize = 20
	closePartyBtn.Text = "X"

	local createBtn = Instance.new("TextButton", PartyContainer)
	createBtn.Size = UDim2.new(0.9, 0, 0, 35)
	createBtn.Position = UDim2.new(0.05, 0, 0, 55)
	createBtn.Font = Enum.Font.GothamBold
	createBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	createBtn.TextSize = 14
	createBtn.Text = "CREATE PARTY"
	ApplyButtonGradient(createBtn, Color3.fromRGB(60, 140, 60), Color3.fromRGB(30, 80, 30), Color3.fromRGB(40, 100, 40))

	local leaveBtn = Instance.new("TextButton", PartyContainer)
	leaveBtn.Size = UDim2.new(0.9, 0, 0, 35)
	leaveBtn.Position = UDim2.new(0.05, 0, 0, 95)
	leaveBtn.Font = Enum.Font.GothamBold
	leaveBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	leaveBtn.TextSize = 14
	leaveBtn.Text = "LEAVE PARTY"
	ApplyButtonGradient(leaveBtn, Color3.fromRGB(160, 60, 60), Color3.fromRGB(100, 30, 30), Color3.fromRGB(120, 40, 40))

	local invBox = Instance.new("TextBox", PartyContainer)
	invBox.Size = UDim2.new(0.6, 0, 0, 35)
	invBox.Position = UDim2.new(0.05, 0, 0, 145)
	invBox.BackgroundColor3 = Color3.fromRGB(15, 15, 18)
	...
	local invBtn = Instance.new("TextButton", PartyContainer)
	invBtn.Size = UDim2.new(0.28, 0, 0, 35)
	invBtn.Position = UDim2.new(0.67, 0, 0, 145)
	invBtn.Font = Enum.Font.GothamBold
	invBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	invBtn.TextSize = 14
	invBtn.Text = "INVITE"
	ApplyButtonGradient(invBtn, Color3.fromRGB(60, 100, 180), Color3.fromRGB(30, 50, 100), Color3.fromRGB(40, 60, 120))

	local pMemScroll = Instance.new("ScrollingFrame", PartyContainer)
	pMemScroll.Size = UDim2.new(0.9, 0, 1, -195)
	pMemScroll.Position = UDim2.new(0.05, 0, 0, 190)
	pMemScroll.BackgroundTransparency = 1
	pMemScroll.ScrollBarThickness = 4
	local pmListLayout = Instance.new("UIListLayout", pMemScroll)
	pmListLayout.Padding = UDim.new(0, 5)
	...
	invBtn.MouseButton1Click:Connect(function() 
		if invBox.Text ~= "" then 
			Network:WaitForChild("PartyAction"):FireServer("Invite", invBox.Text) 
			invBox.Text = ""
		end
	end)

	PartyToggleBtn.MouseButton1Click:Connect(function()
		PartyContainer.Visible = true
		PartyContainer.Position = UDim2.new(0.5, 0, 0.5, 50)
		TweenService:Create(PartyContainer, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, 0, 0.5, 0)}):Play()
	end)
