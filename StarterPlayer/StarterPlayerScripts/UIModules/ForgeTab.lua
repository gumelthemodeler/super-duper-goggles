...
local function CreateStationSquare(parent, rarityColor, isDews, lOrder)
	...
	sq.Size = UDim2.new(0, 90, 0, 90); sq.BackgroundColor3 = Color3.fromRGB(22, 22, 28); sq.Text = ""; sq.LayoutOrder = lOrder
	...
	if not isDews then
		...
		tTxt = Instance.new("TextLabel", tBox); tTxt.Size = UDim2.new(1, 0, 1, 0); tTxt.BackgroundTransparency = 1; tTxt.Font = Enum.Font.GothamBlack; tTxt.TextColor3 = Color3.new(0,0,0); tTxt.TextSize = 11; tTxt.Text = "?"
	end
	...
	local tCon = Instance.new("UITextSizeConstraint", nameLbl); tCon.MaxTextSize = isDews and 18 or 12; tCon.MinTextSize = 8

	local cntLbl = Instance.new("TextLabel", sq); cntLbl.Size = UDim2.new(1, -4, 0, 18); cntLbl.Position = UDim2.new(0, 2, 1, -20); cntLbl.BackgroundTransparency = 1; cntLbl.Font = Enum.Font.GothamBold; cntLbl.TextColor3 = Color3.fromRGB(150, 150, 150); cntLbl.TextSize = 10; cntLbl.TextXAlignment = Enum.TextXAlignment.Center; cntLbl.Text = isDews and "Cost: 0" or "Req: 0"; cntLbl.RichText = true

	return sq, nameLbl, cntLbl, stroke, tBox, tTxt
end

local function CreateMathSym(parent, sym, lOrder)
	local lbl = Instance.new("TextLabel", parent); lbl.Size = UDim2.new(0, 20, 0, 90); lbl.BackgroundTransparency = 1; lbl.Font = Enum.Font.GothamBlack; lbl.TextColor3 = Color3.fromRGB(150, 150, 150); lbl.TextSize = 24; lbl.Text = sym; lbl.LayoutOrder = lOrder
	return lbl
end
...
function ForgeTab.Init(parentFrame, tooltipMgr)
	...
	TopNav.Size = UDim2.new(1, 0, 0, 50); TopNav.BackgroundColor3 = Color3.fromRGB(15, 15, 18); TopNav.ScrollBarThickness = 0; TopNav.ScrollingDirection = Enum.ScrollingDirection.X
	Instance.new("UICorner", TopNav).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", TopNav).Color = Color3.fromRGB(120, 100, 60)
	local navLayout = Instance.new("UIListLayout", TopNav); navLayout.FillDirection = Enum.FillDirection.Horizontal; navLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left; navLayout.VerticalAlignment = Enum.VerticalAlignment.Center; navLayout.Padding = UDim.new(0, 15)
	local navPad = Instance.new("UIPadding", TopNav); navPad.PaddingLeft = UDim.new(0, 15); navPad.PaddingRight = UDim.new(0, 15)

	navLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() TopNav.CanvasSize = UDim2.new(0, navLayout.AbsoluteContentSize.X + 30, 0, 0) end)

	ContentArea = Instance.new("Frame", MainFrame)
	ContentArea.Size = UDim2.new(1, 0, 1, -65); ContentArea.Position = UDim2.new(0, 0, 0, 65); ContentArea.BackgroundTransparency = 1

	local function CreateSubNavBtn(name, text)
		local btn = Instance.new("TextButton", TopNav)
		btn.Size = UDim2.new(0, 180, 0, 36); btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.fromRGB(180, 180, 180); btn.TextSize = 13; btn.Text = text
	...
	-- ==========================================
	-- [[ 1. CRAFTING TAB (PC SIDE-BY-SIDE) ]]
	-- ==========================================
	...
	local ColumnsContainer = Instance.new("Frame", SubTabs["Crafting"])
	ColumnsContainer.Size = UDim2.new(1, 0, 1, 0); ColumnsContainer.BackgroundTransparency = 1
	...
	LeftCol.Size = UDim2.new(0.49, 0, 1, 0); LeftCol.BackgroundTransparency = 1
	...
	local WorkbenchPanel = Instance.new("Frame", LeftCol)
	WorkbenchPanel.Size = UDim2.new(1, 0, 0, 200); WorkbenchPanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25); WorkbenchPanel.LayoutOrder = 1
	...
	FormulaArea.Size = UDim2.new(1, -20, 0, 105); FormulaArea.Position = UDim2.new(0, 10, 0, 35); FormulaArea.BackgroundTransparency = 1
	FormulaArea.ScrollBarThickness = 6; FormulaArea.ScrollingDirection = Enum.ScrollingDirection.X; FormulaArea.BorderSizePixel = 0
	local fLayout = Instance.new("UIListLayout", FormulaArea); fLayout.FillDirection = Enum.FillDirection.Horizontal; fLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center; fLayout.VerticalAlignment = Enum.VerticalAlignment.Center; fLayout.Padding = UDim.new(0, 10); fLayout.SortOrder = Enum.SortOrder.LayoutOrder
	...
	fPlaceholder.Size = UDim2.new(1, 0, 1, 0); fPlaceholder.BackgroundTransparency = 1; fPlaceholder.Font = Enum.Font.GothamMedium; fPlaceholder.TextColor3 = Color3.fromRGB(150, 150, 150); fPlaceholder.TextSize = 14; fPlaceholder.Text = "Select a Blueprint below to view its formula."

	craftBtn = Instance.new("TextButton", WorkbenchPanel)
	craftBtn.Size = UDim2.new(0.8, 0, 0, 40); craftBtn.Position = UDim2.new(0.1, 0, 1, -50); craftBtn.Font = Enum.Font.GothamBlack; craftBtn.TextColor3 = Color3.fromRGB(255, 255, 255); craftBtn.TextSize = 16; craftBtn.Text = "SELECT BLUEPRINT"
	...
	local listTitle = Instance.new("TextLabel", LeftCol)
	listTitle.Size = UDim2.new(1, 0, 0, 30); listTitle.BackgroundTransparency = 1; listTitle.Font = Enum.Font.GothamBlack; listTitle.TextColor3 = Color3.fromRGB(255, 215, 100); listTitle.TextSize = 18; listTitle.Text = "BLUEPRINTS"; listTitle.LayoutOrder = 2
	ApplyGradient(listTitle, Color3.fromRGB(255, 215, 100), Color3.fromRGB(255, 150, 50))

	RecipeList = Instance.new("ScrollingFrame", LeftCol)
	RecipeList.Size = UDim2.new(1, 0, 1, -260); RecipeList.BackgroundTransparency = 1; RecipeList.LayoutOrder = 3; RecipeList.ScrollBarThickness = 6; RecipeList.BorderSizePixel = 0
	local recLayout = Instance.new("UIListLayout", RecipeList); recLayout.Padding = UDim.new(0, 10); recLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	recLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() RecipeList.CanvasSize = UDim2.new(0, 0, 0, recLayout.AbsoluteContentSize.Y + 20) end)

	local RightCol = Instance.new("Frame", ColumnsContainer)
	RightCol.Size = UDim2.new(0.49, 0, 1, 0); RightCol.BackgroundTransparency = 1

	local InvTitleLbl = Instance.new("TextLabel", RightCol)
	InvTitleLbl.Size = UDim2.new(1, 0, 0, 30); InvTitleLbl.BackgroundTransparency = 1; InvTitleLbl.Font = Enum.Font.GothamBlack; InvTitleLbl.TextColor3 = Color3.fromRGB(255, 215, 100); InvTitleLbl.TextSize = 18; InvTitleLbl.Text = "YOUR INVENTORY"

	CraftInvGrid = Instance.new("ScrollingFrame", RightCol)
	CraftInvGrid.Size = UDim2.new(1, 0, 1, -40); CraftInvGrid.Position = UDim2.new(0, 0, 0, 40); CraftInvGrid.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
	local cigLayout = Instance.new("UIGridLayout", CraftInvGrid); cigLayout.CellSize = UDim2.new(0.3, 0, 0, 95); cigLayout.CellPadding = UDim2.new(0.03, 0, 0, 15); cigLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center; cigLayout.SortOrder = Enum.SortOrder.LayoutOrder
	...
	CraftInvGrid.ScrollBarThickness = 6; CraftInvGrid.BorderSizePixel = 0
	cigLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() CraftInvGrid.CanvasSize = UDim2.new(0, 0, 0, cigLayout.AbsoluteContentSize.Y + 30) end)
	...
	local function RenderCrafting()
		...
		for recipeName, recipe in pairs(ItemData.ForgeRecipes) do
			...
			btn.Size = UDim2.new(0.95, 0, 0, 50); btn.Text = ""
			ApplyButtonGradient(btn, Color3.fromRGB(35, 35, 40), Color3.fromRGB(20, 20, 25), Color3.fromRGB(60, 60, 70))

			local tagBox = Instance.new("Frame", btn); tagBox.Size = UDim2.new(0, 16, 0, 16); tagBox.Position = UDim2.new(0, 10, 0, 8); tagBox.BackgroundColor3 = Color3.fromHex(rColor:gsub("#","")); Instance.new("UICorner", tagBox).CornerRadius = UDim.new(0, 4)
			local tagTxt = Instance.new("TextLabel", tagBox); tagTxt.Size = UDim2.new(1, 0, 1, 0); tagTxt.BackgroundTransparency = 1; tagTxt.Font = Enum.Font.GothamBlack; tagTxt.TextColor3 = Color3.new(0,0,0); tagTxt.TextSize = 11; tagTxt.Text = string.sub(resData.Rarity or "C", 1, 1)

			local lbl = Instance.new("TextLabel", btn); lbl.Size = UDim2.new(0.6, 0, 1, 0); lbl.Position = UDim2.new(0, 35, 0, 0); lbl.BackgroundTransparency = 1; lbl.Font = Enum.Font.GothamBold; lbl.TextColor3 = Color3.fromRGB(230,230,230); lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.TextSize = 13
			lbl.Text = recipe.Result

			local statusLbl = Instance.new("TextLabel", btn); statusLbl.Size = UDim2.new(0, 90, 1, 0); statusLbl.Position = UDim2.new(1, -100, 0, 0); statusLbl.BackgroundTransparency = 1; statusLbl.Font = Enum.Font.GothamBlack; statusLbl.TextColor3 = canCraft and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100); statusLbl.TextXAlignment = Enum.TextXAlignment.Right; statusLbl.TextSize = 12; statusLbl.Text = canCraft and "AVAILABLE" or "MISSING"
		...
		for _, item in ipairs(invItems) do
			...
			if count > 0 then
				...
				local accentBar = Instance.new("Frame", card); accentBar.Size = UDim2.new(0, 6, 1, 0); accentBar.BackgroundColor3 = rarityRGB; accentBar.BorderSizePixel = 0; Instance.new("UICorner", accentBar).CornerRadius = UDim.new(0, 4)
				...
				local nameLbl = Instance.new("TextLabel", card); nameLbl.Size = UDim2.new(0.88, 0, 0.5, 0); nameLbl.Position = UDim2.new(0.5, 0, 0.5, 4); nameLbl.AnchorPoint = Vector2.new(0.5, 0.5); nameLbl.BackgroundTransparency = 1; nameLbl.Font = Enum.Font.GothamBold; nameLbl.TextColor3 = Color3.fromRGB(235, 235, 235); nameLbl.TextScaled = true; nameLbl.TextWrapped = true; nameLbl.Text = item.Name; nameLbl.ZIndex = 3
				local tConstraint = Instance.new("UITextSizeConstraint", nameLbl); tConstraint.MaxTextSize = 12; tConstraint.MinTextSize = 8
	...
	local function RenderFusion()
		...
		-- [[ FIX: Clear instruction states in the main fusion boxes ]]
