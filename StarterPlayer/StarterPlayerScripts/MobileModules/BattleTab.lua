...
local pMemScroll -- Used for the Party UI list
...
local function SwitchToPvPMenu(parentFrame)
	-- [[ FIX: Flawless PVP Tab Transition (No Global Navigation Hijacking) ]]
...
function BattleTab.Init(parentFrame)
	...
	-- [[ FIX: Party Container actively blocks background clicks and automatically refreshes ]]
	...
	-- [[ FIX: Added Server Integration for the Party Prompt system ]]
	Network:WaitForChild("PartyUpdate").OnClientEvent:Connect(function(action, data)
		if action == "UpdateList" then
			...
			for _, mem in ipairs(data) do
				local row = Instance.new("Frame", pMemScroll)
				row.Size = UDim2.new(1, -10, 0, 30); row.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
				Instance.new("UICorner", row).CornerRadius = UDim.new(0, 4)

				local lbl = Instance.new("TextLabel", row)
				lbl.Size = UDim2.new(1, -10, 1, 0); lbl.Position = UDim2.new(0, 10, 0, 0); lbl.BackgroundTransparency = 1
				lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 12; lbl.TextColor3 = Color3.new(1,1,1); lbl.TextXAlignment = Enum.TextXAlignment.Left
				lbl.Text = mem.Name .. (mem.IsLeader and " (LEADER)" or "")
		...
		elseif action == "IncomingInvite" then
			...
			prompt.Name = "PartyInvite_" .. senderName
			...
			stroke.Color = Color3.fromRGB(150, 255, 150); stroke.Thickness = 2
			...
			lbl.Text = senderName .. " invited you to a Party!"
			...
			accBtn.MouseButton1Click:Connect(function()
				Network.PartyAction:FireServer("AcceptInvite", senderName)
				prompt:Destroy()
			end)
